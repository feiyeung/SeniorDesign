...
void main()
{
    ...
    UART_1_UartPutString("SV=");
    
    
    
    char str_out[8];
    sprintf(str_out, "%08X", seed);
    UART_1_UartPutString(str_out);
...
//    LCD_PrintInt16(seed);    
    ...
    for (i=0; i<=SEED_VALUE_COUNT; i++) 
    {
        UART_1_UartPutString(" Calc=");
        sprintf(str_out, "%08i", i);
        UART_1_UartPutString(str_out);
        UART_1_UartPutString("\t ");
        UART_1_UartPutString("CRC=");
        uint16 tmp = CRC_ReadCRC();
        sprintf(str_out, "%08X", tmp);
        UART_1_UartPutString(str_out);
        UART_1_UartPutString("\r\n");
    ...
    // LCD_Position(0u,0u);
    UART_1_UartPutString("CRC after Reset:");
