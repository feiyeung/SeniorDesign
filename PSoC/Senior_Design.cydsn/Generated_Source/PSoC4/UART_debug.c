...
static void UART_debug_ScbModeStop(void)
{
    ...
    *******************************************************************************/
    void UART_debug_SetPins(uint32 mode, uint32 subMode, uint32 uartEnableMask)
    {
        ...
        {
            UART_debug_RX_SCL_MOSI_HSIOM_SEL_GPIO,
            UART_debug_TX_SDA_MISO_HSIOM_SEL_GPIO,
            0u,
            0u,
            0u,
            0u,
            ...
            (UART_debug_SCB_MODE_EZI2C == mode))
        {
            ...
            hsiomSel[UART_debug_RX_SCL_MOSI_PIN_INDEX] = UART_debug_RX_SCL_MOSI_HSIOM_SEL_I2C;
            pinsDm  [UART_debug_RX_SCL_MOSI_PIN_INDEX] = UART_debug_PIN_DM_OD_LO;
        #elif (UART_debug_RX_WAKE_SCL_MOSI_PIN)
            hsiomSel[UART_debug_RX_WAKE_SCL_MOSI_PIN_INDEX] = UART_debug_RX_WAKE_SCL_MOSI_HSIOM_SEL_I2C;
            pinsDm  [UART_debug_RX_WAKE_SCL_MOSI_PIN_INDEX] = UART_debug_PIN_DM_OD_LO;
        #else
        #endif /* (UART_debug_RX_SCL_MOSI_PIN) */
        
        #if (UART_debug_TX_SDA_MISO_PIN)
            hsiomSel[UART_debug_TX_SDA_MISO_PIN_INDEX] = UART_debug_TX_SDA_MISO_HSIOM_SEL_I2C;
        ...
        {
        #if (UART_debug_RX_SCL_MOSI_PIN)
            hsiomSel[UART_debug_RX_SCL_MOSI_PIN_INDEX] = UART_debug_RX_SCL_MOSI_HSIOM_SEL_SPI;
        #elif (UART_debug_RX_WAKE_SCL_MOSI_PIN)
            hsiomSel[UART_debug_RX_WAKE_SCL_MOSI_PIN_INDEX] = UART_debug_RX_WAKE_SCL_MOSI_HSIOM_SEL_SPI;
        #else
        #endif /* (UART_debug_RX_SCL_MOSI_PIN) */
        
        #if (UART_debug_TX_SDA_MISO_PIN)
            hsiomSel[UART_debug_TX_SDA_MISO_PIN_INDEX] = UART_debug_TX_SDA_MISO_HSIOM_SEL_SPI;
        #endif /* (UART_debug_TX_SDA_MISO_PIN) */
        
        #if (UART_debug_SCLK_PIN)
            hsiomSel[UART_debug_SCLK_PIN_INDEX] = UART_debug_SCLK_HSIOM_SEL_SPI;
            ...
            if (UART_debug_SPI_SLAVE == subMode)
            {
                ...
                pinsDm[UART_debug_SCLK_PIN_INDEX] = UART_debug_PIN_DM_DIG_HIZ;

            #if (UART_debug_SS0_PIN)
                /* Only SS0 is valid choice for Slave */
                hsiomSel[UART_debug_SS0_PIN_INDEX] = UART_debug_SS0_HSIOM_SEL_SPI;
                pinsDm  [UART_debug_SS0_PIN_INDEX] = UART_debug_PIN_DM_DIG_HIZ;
            ...
            else 
            {
                ...
                pinsDm[UART_debug_SCLK_PIN_INDEX] = UART_debug_PIN_DM_STRONG;

            #if (UART_debug_SS0_PIN)
                hsiomSel [UART_debug_SS0_PIN_INDEX] = UART_debug_SS0_HSIOM_SEL_SPI;
                pinsDm   [UART_debug_SS0_PIN_INDEX] = UART_debug_PIN_DM_STRONG;
                pinsInBuf |= UART_debug_SS0_PIN_MASK;
            #endif /* (UART_debug_SS0_PIN) */
            ...
            #if (UART_debug_SCLK_PIN)
                pinsInBuf |= UART_debug_SCLK_PIN_MASK;
            #endif /* (UART_debug_SCLK_PIN) */
            }
        }
        else /* UART */
        {
            if (UART_debug_UART_MODE_SMARTCARD == subMode)
            {
                /* SmartCard */
            #if (UART_debug_TX_SDA_MISO_PIN)
                hsiomSel[UART_debug_TX_SDA_MISO_PIN_INDEX] = UART_debug_TX_SDA_MISO_HSIOM_SEL_UART;
                pinsDm  [UART_debug_TX_SDA_MISO_PIN_INDEX] = UART_debug_PIN_DM_OD_LO;
            #endif /* (UART_debug_TX_SDA_MISO_PIN) */
            }
            else /* Standard or IrDA */
            {
                ...
                {
                #if (UART_debug_RX_SCL_MOSI_PIN)
                    hsiomSel[UART_debug_RX_SCL_MOSI_PIN_INDEX] = UART_debug_RX_SCL_MOSI_HSIOM_SEL_UART;
                    pinsDm  [UART_debug_RX_SCL_MOSI_PIN_INDEX] = UART_debug_PIN_DM_DIG_HIZ;
                #elif (UART_debug_RX_WAKE_SCL_MOSI_PIN)
                    hsiomSel[UART_debug_RX_WAKE_SCL_MOSI_PIN_INDEX] = UART_debug_RX_WAKE_SCL_MOSI_HSIOM_SEL_UART;
                    pinsDm  [UART_debug_RX_WAKE_SCL_MOSI_PIN_INDEX] = UART_debug_PIN_DM_DIG_HIZ;
                #else
                #endif /* (UART_debug_RX_SCL_MOSI_PIN) */
                }
                ...
                if (UART_debug_UART_MODE_STD == subMode)
                {
                    ...
                    #if (UART_debug_SCLK_PIN)
                        hsiomSel[UART_debug_SCLK_PIN_INDEX] = UART_debug_SCLK_HSIOM_SEL_UART;
                        pinsDm  [UART_debug_SCLK_PIN_INDEX] = UART_debug_PIN_DM_DIG_HIZ;
                    #endif /* (UART_debug_SCLK_PIN) */
                    }
                    ...
                    #if (UART_debug_SS0_PIN)
                        hsiomSel[UART_debug_SS0_PIN_INDEX] = UART_debug_SS0_HSIOM_SEL_UART;
                        pinsDm  [UART_debug_SS0_PIN_INDEX] = UART_debug_PIN_DM_STRONG;
    ...
    #if (UART_debug_SCLK_PIN)
        UART_debug_SET_HSIOM_SEL(UART_debug_SCLK_HSIOM_REG,
                                       UART_debug_SCLK_HSIOM_MASK,
                                       UART_debug_SCLK_HSIOM_POS,
                                       hsiomSel[UART_debug_SCLK_PIN_INDEX]);

        UART_debug_spi_sclk_SetDriveMode((uint8) pinsDm[UART_debug_SCLK_PIN_INDEX]);

        UART_debug_SET_INP_DIS(UART_debug_spi_sclk_INP_DIS,
                                     UART_debug_spi_sclk_MASK,
                                     (0u != (pinsInBuf & UART_debug_SCLK_PIN_MASK)));
    #endif /* (UART_debug_SCLK_PIN) */

    #if (UART_debug_SS0_PIN)
        UART_debug_SET_HSIOM_SEL(UART_debug_SS0_HSIOM_REG,
                                       UART_debug_SS0_HSIOM_MASK,
                                       UART_debug_SS0_HSIOM_POS,
                                       hsiomSel[UART_debug_SS0_PIN_INDEX]);

        UART_debug_spi_ss0_SetDriveMode((uint8) pinsDm[UART_debug_SS0_PIN_INDEX]);

        UART_debug_SET_INP_DIS(UART_debug_spi_ss0_INP_DIS,
                                     UART_debug_spi_ss0_MASK,
                                     (0u != (pinsInBuf & UART_debug_SS0_PIN_MASK)));
    #endif /* (UART_debug_SS0_PIN) */
